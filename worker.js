...
<head>
	...
	<style>
		...
		* {
		...
		.proxy-region-chip:hover {
			border-color: rgba(97, 219, 255, 0.28);
			background: rgba(97, 219, 255, 0.1);
			color: #ffffff;
		}

		.proxy-region-chip.is-active {
			border-color: rgba(97, 219, 255, 0.46);
			background: linear-gradient(135deg, rgba(97, 219, 255, 0.22), rgba(52, 211, 153, 0.14));
			color: #ffffff;
			box-shadow: inset 0 0 0 1px rgba(97, 219, 255, 0.12);
		...
		.proxy-asn-header span {
			color: var(--text-soft);
			font-size: 0.86rem;
			font-weight: 800;
		}

		.proxy-asn-header small {
			color: var(--muted);
			line-height: 1.5;
			text-align: right;
		}
		...
		.proxy-asn-chip.is-active small {
		...
		.proxy-asn-add-btn {
			min-height: 58px;
			border-radius: 18px;
			border: 1px solid rgba(97, 219, 255, 0.2);
			background: rgba(97, 219, 255, 0.1);
			color: #dff9ff;
			font-weight: 800;
			cursor: pointer;
		...
		.proxy-search-form {
			display: grid;
			grid-template-columns: minmax(260px, 1.35fr) minmax(150px, 0.7fr) minmax(140px, 0.55fr) 160px;
			align-items: flex-end;
			gap: 16px;
			margin-top: 18px;
		}
		...
		html[data-theme='light'] .proxy-region-chip,
		html[data-theme='light'] .proxy-detail-item {
			border-color: rgba(95, 123, 150, 0.14);
			background: rgba(255, 255, 255, 0.62);
			color: #365168;
		}

		html[data-theme='light'] .proxy-region-chip:hover {
			border-color: rgba(14, 165, 233, 0.24);
			background: rgba(14, 165, 233, 0.08);
			color: #10253d;
		}

		html[data-theme='light'] .proxy-region-chip.is-active {
			border-color: rgba(14, 165, 233, 0.32);
			background: linear-gradient(135deg, rgba(14, 165, 233, 0.16), rgba(5, 150, 105, 0.1));
			color: #0f5f8e;
			box-shadow: inset 0 0 0 1px rgba(14, 165, 233, 0.08);
		...
		html[data-theme='light'] .proxy-asn-chip small {
			color: #61778f;
		}

		html[data-theme='light'] .proxy-asn-chip.is-active small {
			color: #0f5f8e;
		}

		html[data-theme='light'] .proxy-asn-add-btn {
			border-color: rgba(14, 165, 233, 0.18);
			background: rgba(14, 165, 233, 0.08);
			color: #075985;
		}

		html[data-theme='light'] .proxy-asn-add-btn:hover {
			border-color: rgba(14, 165, 233, 0.28);
			background: rgba(14, 165, 233, 0.12);
			color: #0f5f8e;
		}

		html[data-theme='light'] .proxy-query-preview {
			border-color: rgba(14, 165, 233, 0.16);
			background: rgba(255, 255, 255, 0.62);
		}

		html[data-theme='light'] .proxy-query-preview code {
			color: #075985;
		}
...
<body>
			...
			<section class="surface-card proxy-search-shell" aria-labelledby="proxySearchTitle">
				<div class="proxy-search-header">
					<div>
						...
						<p class="results-subtitle">按端口和国家地区从网络测绘数据库中发现候选 ProxyIP，筛选条件会同步展示，方便复制候选后继续检测可用性。</p>
					</div>
					<div class="guide-badge">60+ 地区</div>
				</div>

				<div class="proxy-search-quick" id="proxyQuickRegions" aria-label="常用地区快捷选择">
					<button class="proxy-region-chip" type="button" data-region-shortcut="HK">香港 HK</button>
					<button class="proxy-region-chip" type="button" data-region-shortcut="JP">日本 JP</button>
					<button class="proxy-region-chip" type="button" data-region-shortcut="SG">新加坡 SG</button>
					<button class="proxy-region-chip" type="button" data-region-shortcut="US">美国 US</button>
					<button class="proxy-region-chip" type="button" data-region-shortcut="DE">德国 DE</button>
					<button class="proxy-region-chip" type="button" data-region-shortcut="NL">荷兰 NL</button>
					<button class="proxy-region-chip" type="button" data-region-shortcut="GB">英国 GB</button>
					<button class="proxy-region-chip" type="button" data-region-shortcut="AU">澳大利亚 AU</button>
				</div>

				<div class="proxy-asn-panel">
					<div class="proxy-asn-header">
						<span>优质 ASN</span>
						<small>可多选；未选择时不限制 ASN，仅保留基础排除规则。</small>
					</div>
					<div class="proxy-search-quick proxy-asn-options" id="proxyAsnOptions" aria-label="优质 ASN 多选"></div>
					<div class="proxy-asn-add">
						<label class="proxy-search-field" for="customAsnInput">
							<span class="field-label">添加 ASN</span>
							<input class="input-control proxy-search-control proxy-search-input" type="text" id="customAsnInput" maxlength="120" placeholder="14061, AS20473" autocomplete="off" inputmode="text">
						</label>
						<button class="proxy-asn-add-btn" id="customAsnAddBtn" type="button">添加</button>
					</div>
				</div>
				...
				<div class="proxy-search-form">
					<label class="proxy-search-field" for="proxyRegionSelect">
						...
						<select class="input-control proxy-search-control" id="proxyRegionSelect">
							<option value="custom" data-label="自定义地区" data-scope="手动输入两位 ISO 国家或地区代码">✍️ 自定义地区 / ISO Code</option>
							<optgroup label="🌏 亚洲与港澳台 / Asia">
								<option value="HK" data-label="香港" data-scope="中国香港，FOFA 使用 region 字段">🇭🇰 香港 HK</option>
								<option value="MO" data-label="澳门" data-scope="中国澳门，FOFA 使用 region 字段">🇲🇴 澳门 MO</option>
								<option value="TW" data-label="台湾" data-scope="中国台湾，FOFA 使用 region 字段">🇹🇼 台湾 TW</option>
								<option value="CN" data-label="中国大陆" data-scope="中国大陆，FOFA 使用 country 字段">🇨🇳 中国大陆 CN</option>
								<option value="JP" data-label="日本" data-scope="东亚常用落地区">🇯🇵 日本 JP</option>
								<option value="KR" data-label="韩国" data-scope="东亚常用落地区">🇰🇷 韩国 KR</option>
								<option value="SG" data-label="新加坡" data-scope="东南亚常用落地区">🇸🇬 新加坡 SG</option>
								<option value="MY" data-label="马来西亚" data-scope="东南亚节点">🇲🇾 马来西亚 MY</option>
								<option value="TH" data-label="泰国" data-scope="东南亚节点">🇹🇭 泰国 TH</option>
								<option value="VN" data-label="越南" data-scope="东南亚节点">🇻🇳 越南 VN</option>
								<option value="ID" data-label="印度尼西亚" data-scope="东南亚节点">🇮🇩 印度尼西亚 ID</option>
								<option value="PH" data-label="菲律宾" data-scope="东南亚节点">🇵🇭 菲律宾 PH</option>
								<option value="KH" data-label="柬埔寨" data-scope="东南亚节点">🇰🇭 柬埔寨 KH</option>
								<option value="IN" data-label="印度" data-scope="南亚常用落地区">🇮🇳 印度 IN</option>
								<option value="PK" data-label="巴基斯坦" data-scope="南亚节点">🇵🇰 巴基斯坦 PK</option>
								<option value="BD" data-label="孟加拉国" data-scope="南亚节点">🇧🇩 孟加拉国 BD</option>
								<option value="LK" data-label="斯里兰卡" data-scope="南亚节点">🇱🇰 斯里兰卡 LK</option>
							</optgroup>
							<optgroup label="🕌 中东 / Middle East">
								<option value="AE" data-label="阿联酋" data-scope="中东常用落地区">🇦🇪 阿联酋 AE</option>
								<option value="SA" data-label="沙特阿拉伯" data-scope="中东节点">🇸🇦 沙特阿拉伯 SA</option>
								<option value="QA" data-label="卡塔尔" data-scope="中东节点">🇶🇦 卡塔尔 QA</option>
								<option value="BH" data-label="巴林" data-scope="中东节点">🇧🇭 巴林 BH</option>
								<option value="KW" data-label="科威特" data-scope="中东节点">🇰🇼 科威特 KW</option>
								<option value="OM" data-label="阿曼" data-scope="中东节点">🇴🇲 阿曼 OM</option>
								<option value="IL" data-label="以色列" data-scope="中东节点">🇮🇱 以色列 IL</option>
								<option value="TR" data-label="土耳其" data-scope="欧亚交界节点">🇹🇷 土耳其 TR</option>
							</optgroup>
							<optgroup label="🌍 欧洲 / EU">
								<option value="GB" data-label="英国" data-scope="西欧常用落地区">🇬🇧 英国 GB</option>
								<option value="DE" data-label="德国" data-scope="西欧常用落地区">🇩🇪 德国 DE</option>
								<option value="FR" data-label="法国" data-scope="西欧常用落地区">🇫🇷 法国 FR</option>
								<option value="NL" data-label="荷兰" data-scope="西欧常用落地区">🇳🇱 荷兰 NL</option>
								<option value="CH" data-label="瑞士" data-scope="西欧节点">🇨🇭 瑞士 CH</option>
								<option value="SE" data-label="瑞典" data-scope="北欧节点">🇸🇪 瑞典 SE</option>
								<option value="NO" data-label="挪威" data-scope="北欧节点">🇳🇴 挪威 NO</option>
								<option value="FI" data-label="芬兰" data-scope="北欧节点">🇫🇮 芬兰 FI</option>
								<option value="DK" data-label="丹麦" data-scope="北欧节点">🇩🇰 丹麦 DK</option>
								<option value="IE" data-label="爱尔兰" data-scope="西欧节点">🇮🇪 爱尔兰 IE</option>
								<option value="ES" data-label="西班牙" data-scope="南欧节点">🇪🇸 西班牙 ES</option>
								<option value="PT" data-label="葡萄牙" data-scope="南欧节点">🇵🇹 葡萄牙 PT</option>
								<option value="IT" data-label="意大利" data-scope="南欧节点">🇮🇹 意大利 IT</option>
								<option value="AT" data-label="奥地利" data-scope="中欧节点">🇦🇹 奥地利 AT</option>
								<option value="BE" data-label="比利时" data-scope="西欧节点">🇧🇪 比利时 BE</option>
								<option value="PL" data-label="波兰" data-scope="中欧节点">🇵🇱 波兰 PL</option>
								<option value="CZ" data-label="捷克" data-scope="中欧节点">🇨🇿 捷克 CZ</option>
								<option value="HU" data-label="匈牙利" data-scope="中欧节点">🇭🇺 匈牙利 HU</option>
								<option value="RO" data-label="罗马尼亚" data-scope="东欧节点">🇷🇴 罗马尼亚 RO</option>
								<option value="BG" data-label="保加利亚" data-scope="东欧节点">🇧🇬 保加利亚 BG</option>
								<option value="GR" data-label="希腊" data-scope="南欧节点">🇬🇷 希腊 GR</option>
								<option value="UA" data-label="乌克兰" data-scope="东欧节点">🇺🇦 乌克兰 UA</option>
								<option value="RU" data-label="俄罗斯" data-scope="欧亚节点">🇷🇺 俄罗斯 RU</option>
							</optgroup>
							<optgroup label="🌎 美洲 / Americas">
								<option value="US" data-label="美国" data-scope="北美常用落地区">🇺🇸 美国 US</option>
								<option value="CA" data-label="加拿大" data-scope="北美常用落地区">🇨🇦 加拿大 CA</option>
								<option value="MX" data-label="墨西哥" data-scope="北美节点">🇲🇽 墨西哥 MX</option>
								<option value="BR" data-label="巴西" data-scope="南美常用落地区">🇧🇷 巴西 BR</option>
								<option value="AR" data-label="阿根廷" data-scope="南美节点">🇦🇷 阿根廷 AR</option>
								<option value="CL" data-label="智利" data-scope="南美节点">🇨🇱 智利 CL</option>
								<option value="CO" data-label="哥伦比亚" data-scope="南美节点">🇨🇴 哥伦比亚 CO</option>
								<option value="PE" data-label="秘鲁" data-scope="南美节点">🇵🇪 秘鲁 PE</option>
							</optgroup>
							<optgroup label="🌏 大洋洲 / OC">
								<option value="AU" data-label="澳大利亚" data-scope="大洋洲常用落地区">🇦🇺 澳大利亚 AU</option>
								<option value="NZ" data-label="新西兰" data-scope="大洋洲节点">🇳🇿 新西兰 NZ</option>
							</optgroup>
							<optgroup label="🌍 非洲 / Africa">
								<option value="ZA" data-label="南非" data-scope="非洲常用落地区">🇿🇦 南非 ZA</option>
								<option value="EG" data-label="埃及" data-scope="北非节点">🇪🇬 埃及 EG</option>
								<option value="NG" data-label="尼日利亚" data-scope="西非节点">🇳🇬 尼日利亚 NG</option>
								<option value="KE" data-label="肯尼亚" data-scope="东非节点">🇰🇪 肯尼亚 KE</option>
								<option value="MA" data-label="摩洛哥" data-scope="北非节点">🇲🇦 摩洛哥 MA</option>
							</optgroup>
						</select>
					</label>
					...
					<label class="proxy-search-field" for="proxyPortSelect">
						...
						<select class="input-control proxy-search-control" id="proxyPortSelect">
							<option value="443">443 标准 HTTPS</option>
							<option value="nonstandard">非标端口</option>
						</select>
					</label>
					<button class="primary-btn proxy-search-btn" id="fofaBtn" type="button">FOFA</button>
				</div>

				<div class="proxy-search-details" aria-live="polite">
					<div class="proxy-detail-item">
						<span>当前地区</span>
						<strong id="proxyRegionSummary">自定义地区</strong>
						<small id="proxyRegionScope">手动输入两位 ISO 国家或地区代码。</small>
					</div>
					<div class="proxy-detail-item">
						<span>查询字段</span>
						<strong id="proxyQueryField">country / region</strong>
						<small id="proxyQueryFieldHint">港澳台使用 region，其余国家使用 country。</small>
					</div>
					<div class="proxy-detail-item">
						<span>端口策略</span>
						<strong id="proxyPortSummary">443 标准 HTTPS</strong>
						<small id="proxyPortHint">优先寻找常见 HTTPS 代理候选。</small>
					</div>
					<div class="proxy-detail-item">
						<span>ASN 策略</span>
						<strong id="proxyAsnSummary">不限 ASN</strong>
						<small id="proxyAsnHint">可选择多个 ASN 组合搜索。</small>
					</div>
				</div>
				<div class="proxy-query-preview">
					<span class="proxy-query-label">FOFA 查询预览</span>
					<code id="fofaQueryPreview">选择地区后会生成查询条件。</code>
				</div>
			</section>
		</main>
	...
	<script>
		...
		const proxyAsnOptions = document.getElementById('proxyAsnOptions');
		const customAsnInput = document.getElementById('customAsnInput');
		const customAsnAddBtn = document.getElementById('customAsnAddBtn');
		const fofaBtn = document.getElementById('fofaBtn');
		const proxyQuickRegions = document.getElementById('proxyQuickRegions');
		const proxyRegionSummary = document.getElementById('proxyRegionSummary');
		const proxyRegionScope = document.getElementById('proxyRegionScope');
		const proxyQueryField = document.getElementById('proxyQueryField');
		const proxyQueryFieldHint = document.getElementById('proxyQueryFieldHint');
		const proxyPortSummary = document.getElementById('proxyPortSummary');
		const proxyPortHint = document.getElementById('proxyPortHint');
		const proxyAsnSummary = document.getElementById('proxyAsnSummary');
		const proxyAsnHint = document.getElementById('proxyAsnHint');
		const fofaQueryPreview = document.getElementById('fofaQueryPreview');
		...
		const FOFA_REGION_FIELD_OVERRIDES = new Set(['HK', 'MO', 'TW']);
		const FOFA_BASE_QUERY = 'server=="cloudflare" && header="Forbidden" && asn!="13335" && asn!="209242"';
		const DEFAULT_FOFA_ASNS = [
			{ asn: '14061', label: 'DigitalOcean' },
			{ asn: '20473', label: 'Vultr' },
			{ asn: '63949', label: 'Akamai Linode' },
			{ asn: '16276', label: 'OVHcloud' },
			{ asn: '24940', label: 'Hetzner' },
			{ asn: '8075', label: 'Microsoft Azure' },
			{ asn: '16509', label: 'Amazon AWS' },
			{ asn: '15169', label: 'Google Cloud' },
			{ asn: '31898', label: 'Oracle Cloud' },
			{ asn: '45102', label: 'Alibaba Cloud' },
			{ asn: '45090', label: 'Tencent Cloud' },
			{ asn: '9009', label: 'M247' }
		];
		...
		let fofaAsnOptions = DEFAULT_FOFA_ASNS.slice();
		let selectedFofaAsns = new Set();
		...
		function getSelectedRegionOption() {
			if (!proxyRegionSelect) return null;
			return proxyRegionSelect.options[proxyRegionSelect.selectedIndex] || null;
		}

		function getRegionQueryField(region) {
			return FOFA_REGION_FIELD_OVERRIDES.has(region) ? 'region' : 'country';
		}

		function buildRegionQuery(region) {
			return /^[A-Z]{2}$/.test(region)
				? getRegionQueryField(region) + '="' + region + '"'
				: '';
		}

		function getFOFAPortQuery(port) {
			if (port === '443') {
				return 'port="443"';
			}
			if (port === 'nonstandard') {
				return '(port!="80" && port!="8080" && port!="8880" && port!="2052" && port!="2082" && port!="2086" && port!="2095" && port!="443" && port!="2053" && port!="2083" && port!="2087" && port!="2096" && port!="8443")';
			}
			return '';
		}

		function getFOFAPortSummary(port) {
			return port === 'nonstandard' ? '非标端口' : '443 标准 HTTPS';
		}

		function getFOFAPortHint(port) {
			return port === 'nonstandard'
				? '排除 Cloudflare 常见标准端口，寻找更少见的候选入口。'
				: '优先寻找常见 HTTPS 代理候选。';
		}

		function normalizeASN(value) {
			const normalized = String(value || '').trim().replace(/^AS/i, '');
			return /^[1-9][0-9]{0,9}$/.test(normalized) ? normalized : '';
		}

		function normalizeASNList(value) {
			const values = Array.isArray(value)
				? value
				: String(value || '').split(/[^0-9A-Za-z]+/);
			const seenAsns = new Set();
			const asns = [];

			values.forEach(function (item) {
				const asn = normalizeASN(item);
				if (!asn || seenAsns.has(asn)) return;
				seenAsns.add(asn);
				asns.push(asn);
			});
		...
		function getSelectedFOFAASNs() {
			return normalizeASNList(Array.from(selectedFofaAsns));
		}

		function getFOFAASNLabel(asn) {
			const option = fofaAsnOptions.find(function (entry) {
				return entry.asn === asn;
			});
			return option ? option.label : '自定义';
		}

		function buildASNQuery(asns) {
			const selectedAsns = normalizeASNList(asns);
			if (!selectedAsns.length) return '';
			return '(' + selectedAsns.map(function (asn) {
				return 'asn=="' + asn + '"';
			}).join(' || ') + ')';
		}

		function buildFOFAQuery(region, port, asns) {
			const regionQuery = buildRegionQuery(region);
			const portQuery = getFOFAPortQuery(port);
			const asnQuery = buildASNQuery(asns);
			if (!regionQuery || !portQuery) return '';
			return [FOFA_BASE_QUERY, regionQuery, portQuery, asnQuery].filter(Boolean).join(' && ');
		}

		function renderFOFAAsnOptions() {
			if (!proxyAsnOptions) return;
			proxyAsnOptions.innerHTML = fofaAsnOptions.map(function (option) {
				const isActive = selectedFofaAsns.has(option.asn);
				return '<button class="proxy-region-chip proxy-asn-chip' + (isActive ? ' is-active' : '') + '" type="button" data-asn-value="' + escapeHtml(option.asn) + '" aria-pressed="' + String(isActive) + '" title="' + escapeHtml(option.label + ' · AS' + option.asn) + '">'
					+ '<span>AS' + escapeHtml(option.asn) + '</span>'
					+ '<small>' + escapeHtml(option.label) + '</small>'
					+ '</button>';
			}).join('');
		}

		function toggleFOFAASN(asn) {
			const normalizedAsn = normalizeASN(asn);
			if (!normalizedAsn) return;

			if (selectedFofaAsns.has(normalizedAsn)) {
				selectedFofaAsns.delete(normalizedAsn);
			} else {
				selectedFofaAsns.add(normalizedAsn);
			}

			renderFOFAAsnOptions();
			updateProxySearchSummary();
		}

		function addCustomASNs(options) {
			if (!customAsnInput) return false;
			const rawValue = customAsnInput.value.trim();
			const asns = normalizeASNList(rawValue);

			if (!asns.length) {
				if (rawValue || !options || options.silent !== true) {
					showToast('请输入有效 ASN，例如 14061 或 AS20473', 'error');
				}
				return false;
			}

			asns.forEach(function (asn) {
				if (!fofaAsnOptions.some(function (option) { return option.asn === asn; })) {
					fofaAsnOptions.push({ asn: asn, label: '自定义 ASN' });
				}
				selectedFofaAsns.add(asn);
			});

			customAsnInput.value = '';
			renderFOFAAsnOptions();
			updateProxySearchSummary();

			if (!options || options.silent !== true) {
				showToast(asns.length > 1 ? '已添加 ' + asns.length + ' 个 ASN' : '已添加 AS' + asns[0]);
			}

			return true;
		}

		function updateQuickRegionState(region) {
			if (!proxyQuickRegions) return;
			proxyQuickRegions.querySelectorAll('[data-region-shortcut]').forEach(function (button) {
				button.classList.toggle('is-active', button.dataset.regionShortcut === region);
				button.setAttribute('aria-pressed', String(button.dataset.regionShortcut === region));
			});
		}

		function updateProxySearchSummary() {
			const selectedOption = getSelectedRegionOption();
			const selectedValue = String(proxyRegionSelect?.value || '').toUpperCase();
			const region = getSelectedFOFARegion({ silent: true });
			const isCustom = selectedValue === 'CUSTOM';
			const port = proxyPortSelect ? proxyPortSelect.value : '';
			const regionLabel = isCustom
				? (region ? '自定义地区 ' + region : '自定义地区')
				: (selectedOption?.dataset?.label || selectedOption?.textContent || selectedValue || '未选择地区');
			const regionScopeText = isCustom
				? '手动输入两位 ISO 国家或地区代码。'
				: (selectedOption?.dataset?.scope || '使用两位国家或地区代码筛选候选节点。');
			const fieldName = region ? getRegionQueryField(region) : 'country / region';
			const regionQuery = region ? buildRegionQuery(region) : '';
			const portQuery = getFOFAPortQuery(port);
			const selectedAsns = getSelectedFOFAASNs();
			const query = region ? buildFOFAQuery(region, port, selectedAsns) : '';
			const asnSummary = selectedAsns.length
				? selectedAsns.map(function (asn) { return 'AS' + asn; }).join(', ')
				: '不限 ASN';
			const asnHint = selectedAsns.length > 1
				? '多个 ASN 会使用 OR 组合搜索。'
				: (selectedAsns.length === 1 ? getFOFAASNLabel(selectedAsns[0]) + ' 精确筛选。' : '未选择时仅排除 Cloudflare 自家 ASN。');

			if (proxyRegionSummary) proxyRegionSummary.innerText = regionLabel;
			if (proxyRegionScope) proxyRegionScope.innerText = regionScopeText;
			if (proxyQueryField) proxyQueryField.innerText = regionQuery || fieldName;
			if (proxyQueryFieldHint) {
				proxyQueryFieldHint.innerText = region
					? (fieldName === 'region' ? '港澳台地区使用 region 字段定位。' : '该地区使用 country 字段定位。')
					: '港澳台使用 region，其余国家使用 country。';
			}
			if (proxyPortSummary) proxyPortSummary.innerText = getFOFAPortSummary(port);
			if (proxyPortHint) proxyPortHint.innerText = portQuery ? getFOFAPortHint(port) : '请选择有效端口策略。';
			if (proxyAsnSummary) proxyAsnSummary.innerText = asnSummary;
			if (proxyAsnHint) proxyAsnHint.innerText = asnHint;
			if (fofaQueryPreview) fofaQueryPreview.innerText = query || '请输入有效地区后生成查询条件。';

			updateQuickRegionState(region || '');
		}
		...
		function getSelectedFOFARegion(options) {
		...
		function openFOFA() {
			if (customAsnInput && customAsnInput.value.trim() && !addCustomASNs({ silent: true })) {
				return;
			}
			...
			const query = buildFOFAQuery(region, port, selectedAsns);
			if (!query) {
		...
		if (proxyQuickRegions) {
			proxyQuickRegions.addEventListener('click', function (event) {
				const button = event.target.closest('[data-region-shortcut]');
				if (!button || !proxyQuickRegions.contains(button) || !proxyRegionSelect) return;

				const region = button.dataset.regionShortcut || '';
				const option = Array.from(proxyRegionSelect.options).find(function (entry) {
					return entry.value === region;
				});
				if (!option) return;

				proxyRegionSelect.value = region;
		...
		if (proxyAsnOptions) {
			proxyAsnOptions.addEventListener('click', function (event) {
				const button = event.target.closest('[data-asn-value]');
				if (!button || !proxyAsnOptions.contains(button)) return;
				toggleFOFAASN(button.dataset.asnValue || '');
			});
		}

		if (customAsnAddBtn) {
			customAsnAddBtn.addEventListener('click', function () {
				addCustomASNs();
			});
		}

		if (customAsnInput) {
			customAsnInput.addEventListener('keydown', function (event) {
		...
		if (proxyPortSelect) {
			proxyPortSelect.addEventListener('change', updateProxySearchSummary);
		}
